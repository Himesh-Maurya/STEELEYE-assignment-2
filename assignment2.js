function highlightHTMLContent(start, end, htmlContent, plainTextContent) {
  let string="";
  let i,j;
  for (i=0, j = 0;i < htmlContent.length && j < plainTextContent.length;i++) {
...
plainTextPositions.forEach((value) => {
  ...
  let end = value.end;
  htmlContent = highlightHTMLContent(start, end, htmlContent, plainText);
});
